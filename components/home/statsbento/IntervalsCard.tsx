...
function VerticalSticks({
  ...
  React.useEffect(() => {
    ...
    items.forEach((it, i) => {
      ...
      if (i % labelStep === 0) ctx.fillText(it.label, x + stickW / 2, baseline + 10);
    });
    ...
    (async () => {
      try {
        ...
        const list = Array.from(by.entries())
          .map(([s,v]) => ({ label: intervalName(s), v1: v.a ? Math.round((100*v.c)/v.a) : 0 }))
          .filter(x => x.v1 > 0 || /^(M2|m3|P5|Octave)$/.test(x.label))
  ...
  return (
    ...
    >
      ...
      {loading ? (
        ...
        <div className="h-[75%] mt-4 flex items-center justify-center text-base text-[#0f0f0f]">No interval attempts yet.</div>
      ) : (
