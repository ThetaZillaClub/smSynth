...
] satisfies ReadonlyArray<CurriculumItem>;
...
export default function CurriculumMenu({ studentName, onStart }: Props) {
  return (
    <div className="min-h-screen w-full flex flex-col bg-gradient-to-b from-[#f0f0f0] to-[#d2d2d2] text-[#0f0f0f]">
      {/* Private route → seed header as authed to avoid any auth fetch */}
      <PrimaryHeader initialAuthed />
                ...
                >
                  <div className="p-5 sm:p-6">
                    <div className="flex items-center justify-between">
                      ...
                      {it.soon && !it.enabled && (
                        <span className="text-[10px] uppercase tracking-wide bg-white/20 px-2 py-1 rounded-full">Coming soon</span>
                      )}
                    </div>

                    <h3 className="mt-4 text-xl sm:text-2xl font-semibold drop-shadow-[0_1px_0_rgba(0,0,0,0.3)]">{it.title}</h3>
                    <p className="mt-1 text-sm sm:text-base text-white/95">{it.subtitle}</p>

                    <div className="mt-5">
                      <div className={[
                          "inline-flex items-center gap-2 rounded-xl px-4 py-2 text-sm font-medium transition",
                          disabled ? "bg-white/20 text-white/80" : "bg-[#f0f0f0] text-[#0f0f0f] shadow-sm hover:bg-white group-hover:translate-x-0.5",
                        ].join(" ")}
                      >
                        {disabled ? "Locked" : "Start"} {!disabled && <span aria-hidden>↗</span>}
                      </div>
                    </div>
                  </div>
      ...
      <footer className="px-6 py-4 text-xs text-[#373737]">Prototype curriculum — more modes unlocking soon.</footer>
    </div>
  );
}
