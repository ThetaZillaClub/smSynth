...
export default function DynamicOverlay({
  ...
  const draw = useCallback(
    (nowMs: number) => {
      ...
      const targetNowSec = isLive ? Math.max(0, (nowMs - (startAtMs as number)) / 1000) : 0;
      ...
      if (isLive && liveBecameRef.current != null && nowMs - liveBecameRef.current < SMOOTH_WINDOW_MS) {
        ...
        smoothNowSecRef.current += (targetNowSec - smoothNowSecRef.current) * alpha;
      ...
      for (let i = 0; i < phrase.notes.length; i++) {
        ...
        const nEnd = n.startSec + n.durSec;
        if (headTime >= n.startSec && headTime < nEnd) activeIdx = i;
      }
      ...
      // ----- Playhead dot (always visible; glide note→note with easeInOutCirc) -----
      const DOT_RADIUS = 7;
      ...
      if (phrase.notes.length === 0) {
        ...
        if (activeIdx >= 0) {
          ...
          const u = easeInOutCirc(uRaw);
          // center-to-center motion; no “bounce” off cell edges
          ...
          // Find the upcoming note index (first i where notes[i].startSec > headTime)
          let upcomingIdx = phrase.notes.findIndex((n) => headTime < n.startSec);
          if (upcomingIdx === -1) upcomingIdx = phrase.notes.length; // after last
          const prevIdx = upcomingIdx - 1;
          let parkMidi: number;
          if (prevIdx >= 0) {
            // during internal rest or after last: park on previous note
            parkMidi = phrase.notes[prevIdx].midi;
          } else {
            // before first: park on first note
            parkMidi = first.midi;
          }
          const y = midiToYCenter(parkMidi, height, minMidi, maxMidi);
          drawDotAtY(y);
          drewDot = true;
        }
      }
      // Safety: if no dot was drawn (edge case), use last known y
      if (!drewDot && lastYRef.current !== null) {
        ctx.fillStyle = PR_COLORS.dotFill;
        ctx.beginPath();
        ctx.arc(xGuide, lastYRef.current, DOT_RADIUS, 0, Math.PI * 2);
        ctx.fill();
        ctx.lineWidth = 1.25;
        ctx.strokeStyle = PR_COLORS.dotStroke;
        ctx.stroke();
