    ...
    (async () => {
      try {
        ...
        if (!cancelled) {
          ...
          setErr(message);
          // optional: keep console for debugging; remove if you prefer zero console noise
          // console.error(e);
        }
      } finally {
  ...
  const Grid = (
    <div className="flex flex-wrap justify-center gap-8 max-w-4xl mx-auto">
      {rows.map((row) => (
        ...
        >
          <div className="w-full aspect-square bg-gray-300 rounded-t-lg overflow-hidden relative">
            {imageUrls[row.id] ? (
              <Image
