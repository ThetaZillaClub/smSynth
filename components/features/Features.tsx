...
export default function Features() {
  ...
  useLayoutEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      gsap
        .timeline({ defaults: { duration: 0.6, ease: 'power3.out' }, delay: FEATURES_START_DELAY })
    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 max-w-7xl px-6">
        {features.map((feature, index) => (
          <div
            key={index}
            className="bg-[#ebebeb] border border-[#d2d2d2] rounded-lg p-6 shadow-md js-feature-card"
          >
